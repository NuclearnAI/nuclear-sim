...
def run_basic_simulation(duration_seconds: int = 300) -> pd.DataFrame:
    ...
    print(f"{'Time (s)':<8} {'Power (%)':<10} {'Fuel T (°C)':<12} {'Coolant T (°C)':<14} {'Feedwater flow rate':<20} {'Electrical Power':<20} \
          {'Status':<10}")
    print("-" * 70)
    power_level = simulator.state.power_level
    power_levels = [100., 98., 96., 88., 93., 91., 87., 80., 75., 70.]
    ...
    for t in range(duration_seconds):
        ...
        if t % 60 == 0:
            ...
            print(f"{simulator.time:<8.0f} {simulator.state.power_level:<10.1f} "
                  ...
                  f"{simulator.secondary_physics.total_feedwater_flow:<20.2f}" 
                  f"{simulator.secondary_physics.electrical_power_output:<20.2f}" f"{status:<10}"
                )
...
simulator.plot_parameters(['primary.coolant_pressure', 
                           'integration.primary_cold_leg_temp',
                           'integration.primary_hot_leg_temp', 'integration.primary_cold_leg_temp',
                           'integration.heat_removal_rate',
                           'secondary.load_demand', 'secondary.electrical_power', 'secondary.thermal_efficiency',
                           'secondary.sg_avg_steam_quality', 'secondary.feedwater_flow','secondary.sg_avg_pressure'])
